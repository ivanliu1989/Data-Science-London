...
set.seed(888)
fit1 <- train(result~., method='rf',data=x,trControl = fitControl,tuneGrid = Grid)
...
################ gbm ###########################
Grid2 <- expand.grid(n.trees=c(100,500,1000),shrinkage=.1,interaction.depth=c(22))
fitControl1 <- trainControl(method="repeatedcv",10,10)
fit2 <- train(result~., method='gbm', data=x_train,
              trControl = fitControl1,tuneGrid = Grid1,metric="ROC",verbose=T)